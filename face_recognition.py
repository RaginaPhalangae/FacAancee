...
class Face_Recognition:
    def __init__(self,root):
        ...
        img_top=Image.open(r"college_images\image21.jpg")
        ...
        img_bottom=Image.open(r"college_images\image22.jpg")
        img_bottom=img_bottom.resize((950,700),Image.ANTIALIAS)
        self.photoimg_bottom=ImageTk.PhotoImage(img_bottom)

        f_lbl=Label(self.root,image=self.photoimg_bottom)
        f_lbl.place(x=651,y=55,width=950,height=700)
        ...
        b1_1.place(x=300,y=600,width=300,height=40)
    ...
    def face_recog(self):
        def draw_boundary(img,classifier,scalefact,minNeighbours,color,text,clf):
            gray_image=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
            features=classifier.detectMultiScale(gray_image,scalefact,minNeighbours)

            coord=[]

            for(x,y,z,w) in features:
                cv2.rectangle(img,(x,y),(x+z,y+w),(0,255,0),3)
                id,predict=clf.predict(gray_image[y:y+w,x:x+z])
                ...
                if confidence>77:
                    ...
                    cv2.rectangle(img,(x,y),(x+z,y+w),(0,0,255),3)
